...
SceneParser::SceneParser(const char *filename) {
    ...
    ambient_light = Vector3f(0, 0, 0); // 环境光项
    num_lights = 0;
    lights = nullptr;
    num_materials = 0;
    materials = nullptr;
...
void SceneParser::parseFile() {
    ...
    while (getToken(token)) {
        ...
        } else if (!strcmp(token, "OrthographicCamera")) { // 正交相机
...
void SceneParser::parseOrthographicCamera() {
    ...
    int height = readInt();
    getToken(token); assert (!strcmp(token, "}"));
    camera = new OrthographicCamera(center, direction, up, width, height, size);
}
...
Light *SceneParser::parsePointLight() {
    ...
    Vector3f color = readVector3f();
    getToken(token);
    assert (!strcmp(token, "falloff"));
    float falloff = readFloat(); // 衰减系数
    getToken(token);
    assert (!strcmp(token, "}"));
